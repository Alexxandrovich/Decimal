...
START_TEST(test_s21_is_greater_1) {
  ...
  ck_assert_int_eq(0, s21_is_greater(a1, a2));
}
END_TEST
...
START_TEST(test_s21_is_greater_3) {
  ...
  ck_assert_int_eq(1, s21_is_greater(a1, a2));
}
END_TEST
...
START_TEST(test_s21_is_greater_12) {
  ...
  for (int i = 0; i < 5; i++) {
    ck_assert(s21_is_greater(val1[i], val2[i]) == answers[i]);
  };
}
END_TEST
...
Suite* s21_is_greater_test(void) {
  Suite* s;
  TCase* tc_core;
  s = suite_create("\033[97m\033[106ms21_is_greater_test\033[0m");
